    let paidAmountInput = document.getElementById('validationCustom02');
    ...
    paidAmountInput.addEventListener('input', function() {
        if (paidAmountInput.value.length > 0) {
            if (numericRegex.test(paidAmountInput.value)) {
                paidAmountInput.classList.remove('is-invalid');
                paidAmountInput.classList.add('is-valid');
            } else {
                paidAmountInput.classList.remove('is-valid');
    ...
    function checkAndSubmit() {
        ...
        let caseNumberInput = document.getElementById('validationCustom03');
        let courtTypeInput = document.getElementById('validationCustom04');
        if ((caseInput.classList.contains('is-valid') && paidAmountInput.classList.contains('is-valid') && caseNumberInput.classList.contains('is-valid') && courtTypeInput.classList.contains('is-valid'))) {
    ...
    function deletePaidCase(element) {
        let id = element.getAttribute('data-id');
        fetch(`/schedulex/paymentreceived/${id}`, {
                method: 'DELETE',
            })
            .then(response => {
                if (response.ok) {
