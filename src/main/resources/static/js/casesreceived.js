    let agreedAmountInput = document.getElementById('validationCustom02');
    ...
    agreedAmountInput.addEventListener('input', function() {
        if (agreedAmountInput.value.length > 0) {
            if (numericRegex.test(agreedAmountInput.value)) {
                agreedAmountInput.classList.remove('is-invalid');
                agreedAmountInput.classList.add('is-valid');
            } else {
                agreedAmountInput.classList.remove('is-valid');
                agreedAmountInput.classList.add('is-invalid');
            }
        } else {
    ...
    function checkAndSubmit() {
        ...
        let caseNumberInput = document.getElementById('validationCustom03');
        let courtTypeInput = document.getElementById('validationCustom04');
        if ((caseInput.classList.contains('is-valid') && agreedAmountInput.classList.contains('is-valid') && caseNumberInput.classList.contains('is-valid') && courtTypeInput.classList.contains('is-valid'))) {

            const casesReceivedDTO = {
                name: document.getElementById("validationCustom01").value,
                caseIdentityNumber: document.getElementById("validationCustom03").value,
                feesAgreed: document.getElementById("validationCustom02").value,
                courtType: document.getElementById("validationCustom04").value
            };

            fetch('/schedulex/casesreceived', {
                    method: 'POST', // Specifies the method to be POST
                    headers: {
                        'Content-Type': 'application/json', // Sets the content type to JSON
                    },
                    body: JSON.stringify(casesReceivedDTO) // Sends the gameInfo object as a JSON string in the request body
                })
                .then(response => {
                    // Checking if the response from the server is ok (status in the range 200-299)
                    if (response.ok) {
                        window.location.href = './casesreceived';
                    }
    ...
    function deleteReceivedCase(element) {
        let id = element.getAttribute('data-id');
        fetch(`/schedulex/casesreceived/${id}`, {
                method: 'DELETE',
            })
            .then(response => {
                if (response.ok) {
